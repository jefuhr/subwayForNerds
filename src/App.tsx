...
export default function App() {
  ...
  const [panel, setPanel] = useState<'stations' | 'themes' | 'alerts' | 'context' | 'filters' | null>(null);
  ...
  useEffect(() => { const pop = () => { setStationId(readStation()); setDestination(''); }; window.addEventListener('popstate', pop); return () => window.removeEventListener('popstate', pop); }, []);
  ...
  const selectStation = (id: string) => {
    setStationId(id); storage.set('station', id); setDestination(''); setTripKey(null); setPanel(null);
  ...
  const onward = [...new Map(visible.flatMap(d => d.onward.map(s => [s.stationId, s] as const))).values()].sort((a, b) => a.name.localeCompare(b.name));
  const winner = destination && !cached ? firstTo(visible, destination, now) : undefined;
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      <a className="wordmark" href={import.meta.env.BASE_URL} aria-label="Subways for Nerds home"><span className="logo-mark"><TrainFront size={24} strokeWidth={2.3} /></span><span>subways<span className="wordmark-bottom">for nerds<span className="brand-dot">.</span></span></span></a>
    ...
    <main className="main-panel">
      ...
      <div className="board-content">
        ...
        <section className="commute-strip" aria-label="Commute comparison"><div className="commute-icon"><ArrowLeftRight size={19} /></div><div className="commute-copy"><strong>First to your stop</strong><span>The local now, or the express in a minute?</span></div><label className="destination-select"><span className="sr-only">Compare arrivals at a downstream station</span><select value={destination} onChange={e => setDestination(e.target.value)}><option value="">Choose a downstream stop</option>{onward.map(s => <option value={s.stationId} key={s.stationId}>{s.name}</option>)}</select><ChevronDown size={15} /></label>{destination && <button className="icon-button" onClick={() => setDestination('')} aria-label="Clear downstream comparison"><X size={16} /></button>}</section>
        {destination && <p className="comparison-note">{winner ? `Earliest reported arrival: ${clockTime(winner.time)}. Direct trains only; allow your own time to reach the platform.` : 'No fresh direct-train prediction for this stop. Unknown arrival times are not ranked.'}</p>}
        ...
        <section id="departures" className="departure-board" tabIndex={-1}>
          <div className="board-toolbar"><div className="board-title"><h2>Departures</h2><span className="count-badge">{visible.length}</span></div><div className="board-actions"><button className="text-button" onClick={() => setPanel('alerts')}><TriangleAlert size={14} />Alerts{alerts.length ? ` (${alerts.length})` : ''}</button><button className={'text-button filter-button ' + (routes.length ? 'active' : '')} onClick={() => setPanel('filters')}><SlidersHorizontal size={15} />Lines{routes.length ? ` (${routes.length})` : ''}</button><button className="icon-button" onClick={refresh} aria-label="Refresh departures"><RefreshCw size={15} /></button></div></div>
          <div className="direction-tabs" role="group" aria-label="Filter by direction">{[['ALL','All directions'],['NORTH','Northbound'],['SOUTH','Southbound']].map(([id, name]) => <button key={id} aria-pressed={direction === id} className={direction === id ? 'selected' : ''} onClick={() => setDirection(id)}>{id === 'NORTH' ? <ArrowUp size={13} /> : id === 'SOUTH' ? <ArrowDown size={13} /> : <Layers3 size={13} />}{name}</button>)}</div>
          ...
          <div className="platform-groups">{sortedGroups.map(group => <PlatformGroup key={group[0].direction + group[0].partId + (group[0].actualTrack || group[0].scheduledTrack || '?')} departures={group} board={board!} now={now} cached={cached} destination={destination} winner={winner?.key} open={setTripKey} />)}</div>
        </section>
    ...
    <Suspense fallback={<div className="panel-loading" role="status">Opening details…</div>}>{tripKey && <TrainDetail tripKey={tripKey} close={() => setTripKey(null)} now={now} />}{panel === 'context' && board && <ContextDetail board={board} close={() => setPanel(null)} now={now} />}</Suspense>
  </div>;
}
...
function PlatformGroup({ departures, board, now, cached, destination, winner, open }: { departures: Departure[]; board: Board; now: number; cached: boolean; destination: string; winner?: string; open: (key: string) => void }) {
  ...
  return <section className="platform-card"><header className="platform-heading"><div className="platform-direction">{first.direction === 'NORTH' ? <ArrowUp size={17} /> : <ArrowDown size={17} />}<h3>{direction}</h3></div><span className="platform-track">{track ? `TRACK ${track}` : 'TRACK UNKNOWN'}<span>{first.actualTrack ? 'reported' : track ? 'scheduled' : 'direction group'}</span></span><div className="platform-subheading"><span>{part.line}</span><span>{first.direction === 'NORTH' ? 'NORTHBOUND' : first.direction === 'SOUTH' ? 'SOUTHBOUND' : 'UNKNOWN DIRECTION'}</span></div></header>
    ...
    <div>{rows.map((d, index) => {
      const time = countdown(d.time, d.timestamp, now, cached), rank = winner === d.key;
      const target = destination ? d.onward.find(s => s.stationId === destination) : undefined;
